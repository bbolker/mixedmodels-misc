...
\citep{biswas2015} & 
\small  MCEM etc. \citep{booth_maximizing_1999,sung_monte_2007,ponciano_hierarchical_2009} \\
\hline
\textbf{Bayesian} & 
\small INLA \citep{Rue+2009} & 
\small MCMC
\citep{Hadfield:2009:JSSOBK:v33i02}
\end{tabular}
...
\begin{frame}
  ...
  \item[hierarchical GLMMs] \citep{lee_generalized_2017} \rflink{spaMM}
    (\fnlink{lme4}{glmer}, \pkglink{glmmML},
    \rflink{glmmADMB}, \rflink{glmmTMB},
...
\begin{frame}
  ...
  \item generally slow but flexible; simplifies inference
  \item Gibbs sampling (\rflink{glmmBUGS}, \rflink{MCMCglmm})
  \item Hamiltonian/hybrid Monte Carlo (\rflink{rstanarm}, \rflink{brms}) 
  \end{itemize}
\end{frame}
...
\item penalized basis functions: \rflink{mgcv}, \rflink{brms}
\end{itemize}
\end{frame}
...
\frametitle{On beyond R}
  \begin{columns}
    \begin{column}{6cm}
      \begin{itemize}
      \item Julia: {\tt MixedModels} package
      \item SAS: PROC MIXED, NLMIXED
      \item AS-REML
      \item Stata (GLLAMM, xtmelogit)
      \item Template Model Builder
      \item HLM, MLWiN
...
\begin{frame}
\frametitle{Challenges 1: speed/size}
...
\item L2 regularization with known variance-covariance: \cite{sweetser_diamond_2017}
\item distributed methods: ???
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Identifying and implementing appropriate model complexity}
\begin{itemize}
\item very easy to build big mixed models: $(n^2+n)/2$ parameters
per $n$-random-slopes random effects term
...
\item penalization for variance components?
