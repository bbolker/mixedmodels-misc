---
title: "choosing and simplifying random effects terms in mixed models"
author: Ben Bolker
date: today
...
An R-centric discussion of what to do to manage the complexity of random effects terms in mixed models.
...
* all terms in the fixed-effect model are included as varying terms where possible, i.e. within every grouping variable where the values of the fixed-effect term vary within (and not just between) groups (*within-subjects* effects in an experimental design)
...
For example, working with the built-in `cbpp` data set where `period` is a four-level factor, the random-effects term `(1+period|herd)` would correspond to estimating four variances and $4 \times 3/2 = six covariances. The exact meanings of these parameters (if they matter in your application) depend on the contrasts used to expand the factor into dummy variables, but the overall model fit (and the estimates and standard errors of any fixed effect parameters) will be the same in any case.

::: {.callout-caution collapse="true"}
## Expand for more details of contrasts etc.
:::
...
For within-subjects categorical effects (factors) with more than two levels, compound-symmetric models are a good simplification approach [@scandolaReliability2024a]. This restricts the correlations among levels to be identical for all pairs of levels (i.e., a correlation matrix with all off-diagonal elements equal to the same constant $\rho$).


the covariance matrices implied by `(period|herd)` vs. `(1|period/herd)` are:
