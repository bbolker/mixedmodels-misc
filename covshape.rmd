...
mk_covprior_2d <- function(..., nsamp = 50, seed = 101) {
    ...
    res <- list(fit = s1$stanfit,
                ...
                covmats = cc,
                ellipses = ee,
                gg = ggplot(ee, aes(x,y, group = row)) + geom_path(alpha=0.5))
...
p_list <- tibble::lst(p_basic, p_fixvar, p_fixvar2, p_round)
```

```{r plot_grid}
plot_grid(plotlist=lapply(p_list, "[[", "gg"))
```
