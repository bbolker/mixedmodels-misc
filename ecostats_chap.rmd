---
title: "GLMM worked examples: additional packages"
author: Ben Bolker
...
Trying out new/additional packages for ecostats examples.
...
library("broom")         ## recent version on Github
library("dotwhisker")
...
Aggregate data as in original example:
```{r tundra_agg,echo=FALSE}
...
```{r dwplot}
dwplot(list(glmmTMB=cmod_glmmTMB,lmer=cmod_lmer))
```
...
tfun <- function(x) {
    t1 <- tidy(x,conf.int=TRUE,effects="fixed")
    t1[,c("term","estimate","conf.low","conf.high")]
}
c0_constr <- tfun(cmod_lme4_L2)
c0_constr <- data.frame(fun="glmer",c0_constr)
c1_constr <- tfun(cmod_blme_L2)
c1_constr <- data.frame(fun="bglmer",c1_constr)
...
ggplot(cmod_constr_Results,aes(x=term,y=estimate,ymin=conf.low,ymax=conf.high,
                               colour=fun))+
...
- try to use `tidyr`/`dotwhisker` throughout
