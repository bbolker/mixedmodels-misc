...
modify_phylo_retrms <- function(rt,phylo,phylonm,
                                phyloZ=phylo.to.Z(phylo),sp,site) {
    ...
    rt[["Ztlist"]][[phylo.pos]] <- do.call(cbind,replicate(length(unique(site)),t(phyloZ),simplify = FALSE))
...
#' 
phylo_glmm <- function(formula,data,family,phylo,phylonm,phyloZ) {
...
phylo_pois_fit <- phylo_glmm(phen_pois~cofactor+(1|phylo)+(1|obs),
...
phylo_gaus_fit <- phylo_lmm(phen~cofactor+(1|phylo)+(1|obs),
                             data=dat_gaus,phylo=phylo,
control=lmerControl(check.nobs.vs.nlev="ignore",check.nobs.vs.nRE="ignore"),
                           phyloZ=phyloZ)
...
pez_gausREspintcor <- communityPGLMM(Y ~ X, data = dat, family = "gaussian", sp = dat$sp,
                       site = dat$site, random.effects = list(re.1,re.2), REML = FALSE,
                       verbose = FALSE)
print(summary(pez_gausREspintcor))
lmer_gausREspintcor <- phylo_lmm(Y ~ X + (1 | sp) + (1|obs)
                                 , data=dat
                                 , phylonm = "sp"
                                 , sp = dat$sp
                                 , site = dat$site
...
```
Phylo-structure is not in (re.2, re.4).
...
- cannot do random slopes 
- cannot do more than one random effect, for example: 1|speices + 1|site 
