...
```{r fitlme4}
opt1 <- lme4::nloptwrap(c(1,0,1),devfun2,lower=c(0,-0.999,0),upper=c(Inf,0.999,Inf),nmax=20)
```
...
```{r mkpoiss}
d$yp <- rpois(nrow(d),exp(d$y))
```

```{r pois_glmmPQL}
MASS::glmmPQL(yp~1,random=~1|f,data=d,
              family=poisson,
              correlation=corAR1(),verbose=FALSE)
```
...
corObj <- nlme:::Initialize(nlme:::corAR1(0, form = ~ 1|f), d)
lme4ord_pois_fit <- strucGlmer(yp ~ 1 + (1|f)+nlmeCorStruct(1, corObj = corObj, sig = 1),
           family=poisson,
           data=d)
```

I'm not actually sure whether I'm specifying the model correctly here,
but the $\phi$ estimate is reasonable ...
(`r round(getCorPar(lme4ord_pois_fit),3)`)
