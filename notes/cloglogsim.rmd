...
set.seed(42)
X <- artSim()
XX <- data.frame(X,fRep=as.character(X$Rep))
write.csv(XX,file="artSim.csv",row.names=FALSE)
```
...
`r round(c(logLik(fit3)-logLik(fit2)))` log-likelihood units worse (!) (this is
true of `nAGQ0initStep=FALSE` with the default optimizer or the BOBYQA optimizer
as well: and things aren't faster either ...
...
timedat <- data.frame(model=names(modList),
                      elapsed=sapply(list(t2,t3,t4,t5,t6,t7),
...
- I'm having trouble getting Julia to evaluate properly in an Rmarkdown chunk (can probably do this but will have to fight with it a bit). The `runr`-based strategy implemented [here](https://rpubs.com/yihui/julia-knitr) *sort of* works, but is flaky
...
@time g1 = glmm(@formula(prop ~ 0+Trt+Trt&x + (x | Rep)), df, Binomial(),
