...
## glmmTMB is not quite modular enough.  Would like to be use do_fit=FALSE
## (or something) and get an object complete enough to finish the fit with ...
## this is a stopgap/proof of concept, copying critical code from inside glmmTMB
...
glmmPQL_fit <- glmmPQL(phen_pois~cofactor,random=~1|allGrp,
        data=dat,
        family="poisson",
...
fit <- fit_TMBstruc(TMBstruc_new)
glmmTMB_res <- matrix(c(fit$fit$par[1:2],exp(fit$fit$par[3:4])^2),
                      ncol=1,
...
- holes
     - `tidy` method for `MCMCglmm_fit` should return SDs for random effects too
     - `lme4` + `profile`?
	 - `glmmTMB`: improve modularity so we can get standard errors etc.
