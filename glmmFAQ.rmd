...
ng <- dim(ranvar)[3]
np <- dim(ranvar)[2]
matrix(ranvar[cbind(rep(seq(np),ng),
             rep(seq(np),ng),
             rep(ng,each=np))],
       byrow=TRUE,
       nrow=ng)
```             
It may be more convenient to use `as.data.frame(cV)` to extract all of the conditional means and standard deviations.
...
**Packages/functions**: `sjstats::r2`, `MuMin::r.squaredGLMM`, `r2glmm` package, the standalone [r2MLM function](https://my.vanderbilt.edu/jasonrights/software/r2MLM), stuff in the `piecewiseSEM` package ...
